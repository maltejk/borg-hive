...
class Repository(BaseModel):
    ...
    * refresh statistics
    ...
    name = models.CharField(
        max_length=256, validators=[RegexValidator(regex=r"^[\w\-\.]+$")]
    )
    ...
    append_only_keys = models.ManyToManyField(
        SSHPublicKey, related_name="append_only_keys", blank=True
    )
    ...
    def get_repo_path(self):
        ...
        """
        return os.path.join(
            settings.BORGHIVE["REPO_PATH"], self.repo_user.name, self.name
        )
    ...
    def get_repo_size(self):
        ...
        if self.is_created():
            ...
            return (
                subprocess.check_output(["du", "-sm", data_path])
                .split()[0]
                .decode("utf-8")
            )
        return None
    ...
    def refresh(self):
        ...
        if self.is_created():
            ...
            statistic = RepositoryStatistic(
                repo_size=self.get_repo_size(), repo_size_unit="MB"
            )
            statistic.repo = self
            statistic.save()
        else:
    ...
    def should_alert(self):
        ...
        if alert_time <= timezone.now():
            LOGGER.debug(
                "Alert Time hit: last update %s is older than %s",
                self.last_updated,
                alert_time,
            )
            firing = True
            ...
            last_alert = self.repositoryevent_set.filter(
                event_type=RepositoryEvent.ALERT, created__gte=self.last_updated
            ).last()

            if last_alert:
                next_alert_interval = last_alert.created + datetime.timedelta(
                    hours=self.owner.alertpreference.alert_interval
                )
                if timezone.now() < next_alert_interval:
                    LOGGER.debug(
                        "Last alert was not too long ago: %s, alert interval is set to: %s",
                        last_alert.created,
                        self.owner.alertpreference.alert_interval,
                    )
                    alert = False
                ...
                #
                alert_expired_time = self.last_updated + datetime.timedelta(
                    days=self.owner.alertpreference.alert_expiration
                )
                if alert_expired_time < timezone.now():
                    LOGGER.debug(
                        "Alert expired: last alert %s is older than set alert expiration: %s",
                        last_alert,
                        self.owner.alertpreference.alert_expiration,
                    )
                    alert = False
        return firing, alert
    ...
    def alert(self):
        ...
        alert = RepositoryEvent(
            event_type=RepositoryEvent.ALERT,
            message=f"Last backup of {self.name} is older than {delta.days} days",
            repo=self,
        )
        alert.save()
...
class RepositoryEvent(BaseModel):
    ...
    def __str__(self):
        return f"RepositoryEvent: {self.event_type}: {self.message}".format(
